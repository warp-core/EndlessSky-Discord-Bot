...
public class MemberEventListener extends ListenerAdapter{
	...
	public void onGuildMemberJoin(GuildMemberJoinEvent event){
		...
		if(!user.isBot()){
			...
			TextChannel rules = guild.getTextChannelsByName(RULES_CHANNEL, true).get(0);
	...
	public void onGuildMemberLeave(GuildMemberLeaveEvent event){
		User user = event.getMember().getUser();
		if(!user.isBot())
			event.getGuild().getTextChannelsByName(MAIN_CHANNEL, true).get(0)
					.sendMessageFormat("%s has just left the server. Goodbye!", user)
					.queue();
	}
	...
	public void onGuildBan(GuildBanEvent event)
	{
		...
		if(!user.isBot())
			event.getGuild().getTextChannelsByName(MAIN_CHANNEL, true).get(0)
					.sendMessageFormat("Bad Boy/Girl %s has just been banned from the server. Cya never!", user)
					.queue();
	}
