...
implements CommandExecutor{
	...
	public void onMemeCommand(MessageChannel channel, String[] args){
		if(args.length == 0){	//random meme
			...
			if(!isImgMeme(args[0])){
				...
				String path = "https://raw.githubusercontent.com/Wrzlprnft/EndlessSky-Discord-Bot/data/memes/" +getImgMemePath(args[0]);
	...
	public void onListmemesCommand(User user, MessageChannel channel, Message message, String[] args){
		if(args.length == 0){
			EmbedBuilder eb = new EmbedBuilder();
			eb.setTitle("Available Memes:", "https://github.com/Wrzlprnft/EndlessSky-Discord-Bot/tree/data");
			eb.addField("Text-based Memes", getMemelist(), false);
			eb.addField("Image-based Memes", getMemelistImgs(), false);
			eb.setThumbnail("https://raw.githubusercontent.com/Wrzlprnft/EndlessSky-Discord-Bot/embed_thumbnails/meme.png");
	...
	private void readMemes() {
		...
		try {
			memes.load(new URL("https://raw.githubusercontent.com/Wrzlprnft/EndlessSky-Discord-Bot/data/memes.txt").openStream());
		...
		try {
			memes.load(new URL("https://raw.githubusercontent.com/Wrzlprnft/EndlessSky-Discord-Bot/data/memeImgs.txt").openStream());
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
